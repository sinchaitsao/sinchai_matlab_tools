% MultiROITractFilter.m
% This program is designed to filter tractography files generated in Ntrack with
% ROI .mat files created in ROIselector and associated programs.  The ouput is 
% a .mat tract file following the format of Ntrack
% Author: Darryl H. Hwang
% Version 1.0
% Last Modified on 12/29/06

function [f_result,filterindex]=sinchai_TractSeedFilterNeuro(roi_loc,tract_loc,varargin)
...
temp=[];
for slice_no=1:sn
    temp_tract=tract{slice_no};
    tcnt=size(temp_tract,2);
    tcnt2=1;
    for i=1:tcnt
        tr=double(temp_tract{i}{1});
%         tr_l=size(tr,1);
        ...
        tr_val=tr;
            ...
            for roi_no=1:length(g_pos_cr)
                ...
                if (roi_count>0) && isand
                    temp{slice_no}{tcnt2}=tract{slice_no}{i};
                    tcnt2=tcnt2+1;
                    break
                end
            end
